          ...
          <div className="space-y-1">
            {order.pickupLocations?.map((location: any, index: number) => {
              const branchItems = order.items.filter(
                (item: any) => item.branch === location.branch || item.branch._id === location.branch._id,
              )

              if (branchItems.length === 0) return null

              const packedItems = branchItems.filter((item: any) => item.status === "packed").length
              const totalItems = branchItems.length

              return (
                <div key={index} className="flex justify-between text-xs">
                  <span className="text-gray-600">Branch {index + 1}:</span>
                  <span className={packedItems === totalItems ? "text-green-600" : "text-orange-600"}>
