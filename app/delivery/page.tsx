...
export default function DeliveryDashboard() {
  ...
  useEffect(() => {
    ...
    fetchDeliveredCount(storedUserId); // ✅ Add this line
  }, [router]);
  ...
  const fetchDeliveredCount = async (userId: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/delivery-partners/${userId}/delivered-count`
      );
      if (!res.ok) throw new Error("Failed to fetch delivered count");
      const data = await res.json();
      setDeliveredCount(data.deliveredOrderCount || 0);
    } catch (error) {
      console.error("Failed to fetch delivered count:", error);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <DeliveryHeader
        userId={userId}
        deliveredCount={deliveredCount} // ✅ Pass the prop
        onRefresh={() => {
          fetchAvailableOrders(userId);
          fetchDeliveredCount(userId); // refresh this too
        }}
